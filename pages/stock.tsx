  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 mb-4 sm:mb-6">
        <h2 className="text-lg sm:text-xl font-bold mb-4">Stock Summary by Material</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {Object.entries(stockSummary).map(([material, area]) => (
            <div key={material} className="border border-gray-200 rounded-lg p-4">
              <h3 className="font-semibold text-base sm:text-lg text-gray-700 break-words">{material}</h3>
              <p className="text-xl sm:text-2xl font-bold text-blue-600">{area.toFixed(2)} m²</p>
              <p className="text-xs sm:text-sm text-gray-500">Total area used</p>
            </div>
          ))}
        </div>
      </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-2 sm:px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Quantity</th>
                  ...
                  <th className="px-2 sm:px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Leftover</th>
                  <th className="px-2 sm:px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Sheet Used</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {data.map((row, index) => (
                  <tr key={index} className="hover:bg-gray-50">
                    <td className="px-2 sm:px-4 py-3 sm:py-4 whitespace-nowrap text-xs sm:text-sm font-medium text-gray-900">{row.Material}</td>
                    ...
                    <td className="px-2 sm:px-4 py-3 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-900">{row.Qty}</td>
                    ...
                    <td className="px-2 sm:px-4 py-3 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-900">{row['Leftover Area (m²)']?.toFixed(2)} m²</td>
